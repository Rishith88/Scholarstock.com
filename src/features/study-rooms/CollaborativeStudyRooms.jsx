...
export default function CollaborativeStudyRooms() {
  ...
  const joinByCode = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) { showToast("Joined!", "success"); setShowJoin(false); setInviteCode(""); await enterRoom(data.room); }
    ...
    <div className="sr-list">
      <div className="sr-list-header">
        <div>
          ...
          <p style={{ color: "var(--muted)", fontSize: ".85rem" }}>Real-time study rooms with shared notes, whiteboard and chat</p>
        </div>
      ...
      <div className="sr-grid">
        {displayedRooms.length === 0 ? (
          <div className="sr-empty">
            ...
            <p>{tab === "my" ? "You have not joined any rooms yet." : "No public rooms found."}</p>
        ...
        <div className="sr-modal-overlay" onClick={() => setShowCreate(false)}>
          <div className="sr-modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={createRoom}>
              ...
              <div className="sr-form-group"><label style={{ display: "flex", alignItems: "center", gap: ".5rem", cursor: "pointer" }}><input type="checkbox" checked={createForm.isPrivate} onChange={e => setCreateForm(p => ({ ...p, isPrivate: e.target.checked }))} />Private room</label></div>
